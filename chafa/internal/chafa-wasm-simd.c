...
/* ============================================================================
 * OPTIMIZED: Process 16 pixels per iteration (4x faster than 4-pixel version)
 * ============================================================================ */
gint
                                  ...
                                  const guint32 *sym_mask_u32)
{
    ...
    /* Process 16 pixels per iteration (4x unroll) */
...
/* ============================================================================
 * NEW: Generate bitmap from cell (8 pixels per iteration with bitmask)
 * ============================================================================ */
guint64
...
/* ============================================================================
 * OPTIMIZED: Mean colors extraction with per-channel accumulation
 * ============================================================================ */
void
chafa_extract_cell_mean_colors_wasm_simd (const ChafaPixel *pixels, ChafaColorAccum *accums_out,
                                           const guint32 *sym_mask_u32)
{
    /* Accumulate RGBA separately for FG and BG */
    ...
    for (i = 0; i < CHAFA_SYMBOL_N_PIXELS; i += 4)
    {
        ...
        /* Widen and accumulate per channel */
...
chafa_color_accum_div_scalar_wasm_simd (ChafaColorAccum *accum, guint16 divisor)
{
    ...
    /* Multiply and take high bits: (a * b + 0x4000) >> 15 */
...
/* Calculate color difference for 4 colors at once */
gint
